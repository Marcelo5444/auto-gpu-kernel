...
| `stride N` | `modal run scripts/run_modal.py --stride N` | Iteration (default `N=8` → ~16 workloads) |
...
Report back: pass/fail counts, absolute kernel latency (min / average / median / max, split small vs large when both are present), max abs/rel error, reference latency.
