...
Iteratively improve `solution/triton/indexer_fused.py`. Rules in `CLAUDE.md` are non-negotiable.

## Loop

IMPORTANT: Make sure research agent is called every 5-10 experiments to ensure we are not going in circles.

1. **Assess.** Read `indexer_fused.py`, `indexer_baseline.py`, `experiments/summary.md`, `experiments/LESSONS.md`. For directly relevant prior attempts, read `experiments/exp_N/result.md`. If the highest-numbered folder has `plan.md` but no `result.md`, implement that plan — it's reserved (see §Folder reservation).
...
3. **Implement.** One optimization. Preserve DPS (don't allocate `topk_indices` inside the kernel; pad with `-1`).

4. **Validate.** `/benchmark quick` (2 workloads: smallest + largest — catches shape-assumption bugs such as `batch_size = 1` assumptions). Fix compile/correctness before proceeding.

5. **Measure.** `/benchmark stride 8`. Before trusting the number:
   ...
   - **Sub-5% deltas are noise on cross-VM comparison.** Confirm with `modal run scripts/ab_benchmark.py::run --a experiments/exp_<prev-best>/indexer_fused.py`.
...
7. **Decide.**
   - **Clear win** (≥5% on stride 8, or A/B-confirmed): keep, continue.
   - **Marginal**: A/B confirm or revert.
   - **Regression**: revert, try different axis.
   - **Plateau/stuck**: see §Research-agent triggers.

8. **Budget.** `stride 8` per iteration is the default (~2-3 min). `full` only when you have a confirmed new best and want the real number, or every ~10 iterations as a drift check.

## When stuck — investigate before guessing

Write a targeted ablation: isolate one component (time scoring vs top-K selection vs page-table remap; swap `tf32x3` ↔ `ieee`; time with `topk=256` vs `2048`; skip FP8 dequant with random data). Log it as its own experiment. If profiling data would resolve the question, add CUDA-event instrumentation gated by an env var (e.g., `DSA_PROFILE=1`) and log the timing breakdown — these entries are some of the most valuable later.
