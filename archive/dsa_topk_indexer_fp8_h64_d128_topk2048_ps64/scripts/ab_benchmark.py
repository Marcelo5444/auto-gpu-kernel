...
Usage:
    modal run scripts/ab_benchmark.py::run --a experiments/exp_N/indexer_fused.py

A is the baseline (e.g., previous best). B is always the current
solution/triton/indexer_fused.py. Both are packed identically and run
...
def _pack(candidate_py: Path, tag: str) -> Solution:
    """Pack a Solution using candidate_py as indexer_fused.py."""
    with tempfile.TemporaryDirectory() as tmp:
        src = Path(tmp) / "triton"
        src.mkdir()
        shutil.copy(TRITON_DIR / "indexer_baseline.py", src / "indexer_baseline.py")
        shutil.copy(candidate_py, src / "indexer_fused.py")
        spec = BuildSpec(
            language="triton",
            target_hardware=["cuda"],
            entry_point="indexer_fused.py::kernel",
            binding=None,
        )
...
def run_ab(sol_a: Solution, sol_b: Solution, stride: int = 8) -> dict:
...
def run(a: str, stride: int = 8):
    a_path = Path(a).resolve()
    b_path = TRITON_DIR / "indexer_fused.py"
