...
def score_kernel(
    ...
    # Exp 43: use tl.make_block_ptr for the large Q and K loads to carry shape + contiguity
    # metadata to the compiler — on Blackwell Triton this is the canonical idiom for TMA-lowered
    # async loads. Order (1, 0) = D-axis is the contiguous inner dim.
    ...
    k_bp = tl.make_block_ptr(
        base=k_fp8_ptr + page_id * stride_kp,
        shape=(BLOCK_T, BLOCK_D),
...
@triton.jit
def score_kernel_flat(
    q_ptr, k_fp8_ptr, k_scale_ptr, w_ptr,
    seq_lens_ptr, block_table_ptr, scores_ptr,
    b_lookup_ptr, p_lookup_ptr,
    stride_qb, stride_qh, stride_qd,
    stride_kp, stride_kt, stride_kd,
    stride_ksp, stride_kst,
    stride_wb, stride_wh,
    ...
    # Exp 45: flat active-only grid driven by host-precomputed (pid_b, pid_p) lookup
    # tables. No early-return branch — host filtered inactive (token_start >= seq_len)
    # programs. Sentinel positions beyond active range are covered by torch.full pre-fill.
    pid = tl.program_id(0)
    pid_b = tl.load(b_lookup_ptr + pid)
    pid_p = tl.load(p_lookup_ptr + pid)
    ...
    final = tl.sum(scores_tile, axis=0) * scale
