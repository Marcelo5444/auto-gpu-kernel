...
with workload-specialized fast path for max_num_pages == 1 (exp 20).
"""
...
def fast_small_kernel(
    ...
    # Skips torch.topk (~25 µs dispatch) and remap_kernel (~19 µs dispatch).
    ...
    # Packed monotone-bits sort (descending on score).
    ...
    # Overwrite first min(seq_len, BLOCK_T) positions with real remapped token_idx.
...
def kernel(q_index_fp8, k_index_cache_fp8, weights, seq_lens, block_table,
    ...
    # Fuses score + sort + remap, skipping torch.topk + remap_kernel dispatch (~44 µs).
    if max_num_pages == 1:
        fast_small_kernel[(batch_size,)](
            ...
            BLOCK_H=H, BLOCK_D=D, BLOCK_T=page_size, TOPK=topk,
        )
        return
