"""
DSA TopK Indexer — Triton score kernel + Triton remap kernel (exp 13).
"""
...
def kernel(q_index_fp8, k_index_cache_fp8, weights, seq_lens, block_table,
    ...
    # Shrink the scanned region to the actual max seq_len across batches.
    # Skip the .item() sync when max_scored <= topk (nothing to gain on torch.topk K).
    if max_scored > topk:
    ...
    scores = torch.empty(
        (batch_size, max_scored_eff), device=q_index_fp8.device, dtype=torch.float32
    )
