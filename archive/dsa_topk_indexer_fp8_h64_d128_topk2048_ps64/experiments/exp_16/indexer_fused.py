"""
DSA TopK Indexer — Triton score kernel + Triton remap kernel (exp 16).

Adds a module-level scores-buffer pool keyed by (B, max_scored, device) to
eliminate the ~9 µs `torch.empty` overhead per call. Score_kernel overwrites
every position of the buffer, so reuse is safe.
"""
...
def kernel(q_index_fp8, k_index_cache_fp8, weights, seq_lens, block_table,
    ...
    cache_key = (batch_size, max_scored, device.type, device.index)
    scores = _scores_cache.get(cache_key)
    if scores is None:
        scores = torch.empty(
            (batch_size, max_scored), device=device, dtype=torch.float32
        )
        _scores_cache[cache_key] = scores
