"""
DSA TopK Indexer — Triton score kernel + Triton remap kernel (exp 18).

Exp 18: Promote BLOCK_T 64→128 by handling TWO adjacent pages per program,
concatenating the two K tiles into a [128, 128] tile for a single 64×128×128
MMA (vs current 64×64×128). Halves the score_kernel grid.
"""
...
def score_kernel(
    ...
    BLOCK_T2: tl.constexpr,      # two-page tile (128)
):
    ...
    # Two adjacent pages per program. Clamp the second for odd max_num_pages;
    # the duplicate-load tail is masked to -1e30 by `abs_t < seq_len` below.
    page_p0 = pid_p * 2
    ...
    k0 = tl.load(k_fp8_ptr + k_off_0)  # [64, 128]
    k1 = tl.load(k_fp8_ptr + k_off_1)  # [64, 128]

    # Concat along token axis: [64,128] + [64,128] → [128, 128].
    # Use tl.join + tl.trans + tl.reshape (same pattern as exp 15 for uint64;
    # tl.cat is known to fail the MLIR pipeline per LESSONS.md).
    k_joined = tl.join(k0, k1)                 # [64, 128, 2]
    k_permuted = tl.trans(k_joined, 2, 0, 1)   # [2, 64, 128]
    k_fp8 = tl.reshape(k_permuted, [BLOCK_T2, BLOCK_D])  # [128, 128]

    s_off_0 = page_id_0 * stride_ksp + t_offs * stride_kst
    s_off_1 = page_id_1 * stride_ksp + t_offs * stride_kst
    scale_0 = tl.load(k_scale_ptr + s_off_0)  # [64]
    scale_1 = tl.load(k_scale_ptr + s_off_1)  # [64]
    scale_joined = tl.join(scale_0, scale_1)          # [64, 2]
    scale_permuted = tl.trans(scale_joined, 1, 0)     # [2, 64]
    scale = tl.reshape(scale_permuted, [BLOCK_T2])    # [128]

    w_off = pid_b * stride_wb + h_offs * stride_wh
    w = tl.load(w_ptr + w_off)

    # Single 64×128×128 MMA.
    scores = tl.dot(q_fp8, tl.trans(k_fp8), out_dtype=tl.float32)  # [64, 128]

    scores = tl.maximum(scores, 0.0)
    scores = scores * w[:, None]

    final = tl.sum(scores, axis=0) * scale  # [128]

    abs_t = token_start + t2_offs
...
def kernel(q_index_fp8, k_index_cache_fp8, weights, seq_lens, block_table,
    ...
    # Pad scores to a multiple of 128 = 2*page_size. This handles odd
    # max_num_pages cleanly (tail program writes 128 tokens, some of which
    # fall beyond the real max_scored = max_num_pages * page_size but are
    # always masked to -1e30 by `abs_t < seq_len`).
    pages_per_prog = 2
