...
# Exp 31: module-level scores pool attempt. Reverted — tied A/B, no gain.
_SCORES_POOL = None
_SCORES_POOL_CAP = 0
...
def kernel(q_index_fp8, k_index_cache_fp8, weights, seq_lens, block_table,
    ...
    if need > _SCORES_POOL_CAP:
        _SCORES_POOL = torch.empty(
            max(need, 512 * 1024), device=q_index_fp8.device, dtype=torch.float32
        )
