...
def radix_topk_kernel(
    ...
    # Pack both prefixes into one cumsum: hi16 = strict_prefix, lo16 = tie_prefix.
    # BLOCK_N ≤ 8192 ≤ 0xFFFF so neither half overflows.
    ...
    # Strict positions go to [0, strict_count); valid ties to [strict_count, topk).
...
@triton.jit
def fused_score_radix_kernel(
    q_ptr, k_fp8_ptr, k_scale_ptr, w_ptr,
    seq_lens_ptr, block_table_ptr, scores_ptr, topk_indices_ptr,
    stride_qb, stride_qh, stride_qd,
    stride_kp, stride_kt, stride_kd,
    stride_ksp, stride_kst,
    stride_wb, stride_wh,
    ...
    # Exp 39: fuse score_kernel + radix_topk_kernel into one kernel.
    # Grid = (batch_size,), one program per batch. Each program loads Q once,
    # loops over pages writing scores to HBM scratch, then runs radix-select
    # + scatter on the same batch. Eliminates inter-kernel dispatch gap and
    # shrinks the page loop to `cdiv(seq_len, page_size)` (vs max_num_pages).
    pid_b = tl.program_id(0)
    seq_len = tl.load(seq_lens_ptr + pid_b)
    ...
    num_pages_for_seq = tl.cdiv(seq_len, page_size)
    for pid_p in tl.range(0, num_pages_for_seq, num_stages=2):
        token_start = pid_p * page_size
        page_id = tl.load(block_table_ptr + pid_b * stride_btb + pid_p * stride_btp).to(tl.int64)
        ...
        scores_tile = tl.dot(q_fp8, tl.trans(k_fp8), out_dtype=tl.float32)
        scores_tile = tl.maximum(scores_tile, 0.0)
        scores_tile = scores_tile * w[:, None]
        final_tile = tl.sum(scores_tile, axis=0) * scale

        abs_t = token_start + t_offs
        in_bounds_t = abs_t < seq_len
        final_tile = tl.where(in_bounds_t, final_tile, -1e30)

        score_off = pid_b * stride_sb + abs_t * stride_sn
        tl.store(scores_ptr + score_off, final_tile)

    # --- RADIX PHASE (byte-for-byte from radix_topk_kernel scoring branch) ---
    offs = tl.arange(0, BLOCK_N)
    # Tighter than max_scored: only consider real token positions.
    in_bounds = offs < seq_len
    score_ptrs = scores_ptr + pid_b * stride_sb + offs * stride_sn
    scores = tl.load(score_ptrs, mask=in_bounds, other=float('-inf'))

    # Exp 37 carry-over: issue block_table load early for HBM/compute overlap.
    page_idx = offs // page_size
    offset = offs % page_size
    page_idx_clamped = tl.minimum(page_idx, num_pages_for_seq - 1)
    bt_ptrs = block_table_ptr + pid_b * stride_btb + page_idx_clamped * stride_btp
    global_page = tl.load(bt_ptrs, mask=in_bounds, other=0).to(tl.int64)
    token_idx = (global_page * page_size + offset).to(tl.int32)

    # fp32 → monotone uint32.
    score_bits = scores.to(tl.uint32, bitcast=True)
    sign = score_bits >> 31
    xor_mask = tl.where(
        sign != 0,
