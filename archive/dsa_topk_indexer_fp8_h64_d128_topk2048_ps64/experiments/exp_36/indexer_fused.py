...
def radix_topk_kernel(
    ...
    # Exp 36 (REVERTED): tried threshold = 0x80000000 init + loop from i=1 to skip iter 0.
    # Correctness failure: weights can be negative, so scores can be negative (mono < 0x80000000).
    # When count of non-neg scores < topk, original iter 0 correctly rejects bit 31;
    # my optimization forced bit 31 set, producing final_mask with < topk True entries
    # leaving garbage in topk_indices output slots → "out-of-range indices" errors.
