...
def _fused_split_combine_kernel(
    ...
    # ===== Atomic barrier =====
    # Per-slot counters at 128-byte stride (32 int32s per slot) — each split
    # CTA writes to its own cache line so the B200 L2 atomic unit's lanes can
    # process the 8 increments in parallel rather than serialising on one
    # line. Combine spins on the min over all slots (all must reach gen).
    SLOT_STRIDE: tl.constexpr = 32  # 128 bytes / sizeof(int32)
    slot_self = Counter_ptr + t * NUM_SPLITS * SLOT_STRIDE + s * SLOT_STRIDE
    tl.atomic_add(slot_self, 1, sem="release")

    slot_base = Counter_ptr + t * NUM_SPLITS * SLOT_STRIDE
    slot_ptrs = slot_base + tl.arange(0, NUM_SPLITS) * SLOT_STRIDE
    vals = tl.load(slot_ptrs, volatile=True)
    min_val = tl.min(vals, axis=0)
    while min_val < target_count:
        vals = tl.load(slot_ptrs, volatile=True)
        min_val = tl.min(vals, axis=0)
    tl.debug_barrier()
...
def _get_counter_and_target(num_tokens: int, device: torch.device, num_splits: int):
    ...
    if cached is None:
        # num_tokens × num_splits slots, 32 int32s (128 bytes) per slot — one
        # cache line per slot so atomic increments don't serialise on line
        # ownership. Each slot's leading i32 is the per-(t,s) counter; the
        # padding tail is unused but reserves the cache line.
        cached = torch.zeros(num_tokens * num_splits * 32, dtype=torch.int32, device=device)
