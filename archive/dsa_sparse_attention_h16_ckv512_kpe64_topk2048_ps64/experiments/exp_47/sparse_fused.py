...
def _fused_split_combine_kernel(
    ...
    for bn in range(0, max_bn, BLOCK_N):
        ...
        logits = tl.dot(q_nope, tl.trans(kc), input_precision="ieee")
        logits = tl.dot(q_pe, tl.trans(kp), acc=logits, input_precision="ieee")
        ...
        acc = acc * alpha[:, None]
        acc = tl.dot(p.to(tl.bfloat16), kc, acc=acc, input_precision="ieee")
        m_i = m_new
