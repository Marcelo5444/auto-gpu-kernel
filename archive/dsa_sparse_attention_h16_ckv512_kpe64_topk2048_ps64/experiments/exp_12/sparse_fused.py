...
Hybrid dispatch. Two regimes from exp_10 characterization:
  * T ≤ 2: fused single-launch D-parallel kernel. Eliminates one of two
    8-µs launch barriers and the 2 MB partial_acc HBM round-trip. Per-CTA
    work (full-TOPK scan) is small enough on tiny T that launch savings
    dominate.
  * T ≥ 3: split + combine (exp_9). Each split-CTA owns 1/NUM_SPLITS of
    TOPK; a D-parallel combine kernel merges. Preserves SM-level
    parallelism on large T, where the fused design would replicate the
    expensive Q@K^T logits compute 8× across D-programs.

Per-regime specialization. Threshold is empirical: exp_10 showed T≤2
wins 36–46% with fused while T≥6 regresses 2–3.5×.
"""
...
def _split_attn_kernel(
    ...
    tl.store(pacc_ptrs, acc.to(tl.bfloat16))
...
def _combine_kernel(
    ...
    for si in tl.static_range(NUM_SPLITS):
        ...
        acc_si = tl.load(pacc_ptr).to(tl.float32)
...
def kernel(q_nope, q_pe, ckv_cache, kpe_cache, sparse_indices, sm_scale, output, lse):
    ...
    partial_acc = torch.empty((num_tokens, NUM_SPLITS, H, D_ckv), dtype=torch.bfloat16, device=device)
