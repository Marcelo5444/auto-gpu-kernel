...
def _fused_split_combine_kernel(
    ...
    # exp_53 variant C (final tested): two-pass (pre-compute m_global, then
    # offline-scaled serial accumulate). See result.md for A/B/C variants.
    d = s
    ...
    pm_all_ptrs = (
        Partial_m_ptr
        + t * stride_pm_t
        + offs_s[:, None] * stride_pm_s
        + offs_h[None, :] * stride_pm_h
    )
    pm_all = tl.load(pm_all_ptrs)
    m_global = tl.max(pm_all, axis=0)
    m_global_safe = tl.where(m_global == NEG_INF, 0.0, m_global)
    ...
    for si in tl.static_range(NUM_SPLITS):
        ...
        scale_si = tl.where(m_si == NEG_INF, 0.0, tl.exp2(m_si - m_global_safe))
        l_global = l_global + l_si * scale_si
        acc_comb = acc_comb + acc_si * scale_si[:, None]
