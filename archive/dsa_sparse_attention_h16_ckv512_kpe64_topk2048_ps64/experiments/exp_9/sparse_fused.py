...
def _split_attn_kernel(
    ...
    # Precompute dynamic loop bound: count valid indices in this split, then
    # loop only over ceil(num_valid / BLOCK_N) blocks. Indices are contiguous
    # with padding as a suffix — only the first num_valid entries are live.
    # Keeps compute unconditional inside the hot loop (num_stages pipelining
    # intact), while skipping all-padding blocks entirely.
    ...
    for bn in range(0, max_bn, BLOCK_N):
        ...
        # Guard against all-padding tail block: -inf - -inf would be NaN.
...
def _combine_kernel(
    ...
    # D_ckv-parallel combine: grid = (T, D_CKV // BLOCK_D). Each program
    # handles BLOCK_D channels. m_global/l_global are per-head only, so their
    # state is replicated trivially across d-programs (negligible redundant
    # compute). Only d==0 writes LSE to avoid store races.
    t = tl.program_id(0)
    d = tl.program_id(1)
