"""
Triton Kernel for DSA Sparse Attention — exp_25 (PDL + inline PTX, reverted).

Hybrid dispatch:
  * T ≤ 2: single-launch D-parallel fused kernel (`_fused_attn_kernel`).
  * T ≥ 3: single-launch "split+combine-in-one-kernel" kernel
    (`_fused_split_combine_kernel`).

exp_25 additions on top of exp_24:
  - `tl.inline_asm_elementwise("griddepcontrol.launch_dependents;", ...)` at
    the epilogue of both kernels, closing the PDL signal loop that exp_21
    found absent.
  - `launch_pdl=True` on both launch sites.

Reverted — A/B 6/12 wins vs exp_24, mean Δ ≈ 0. CUPTI measures individual
kernel duration so PDL's cross-kernel overlap is not observable in the
benchmark harness. This snapshot is kept for reproducibility.
"""
...
def _fused_split_combine_kernel(
    ...
    # PDL signal: tell the driver the next queued kernel may begin its
    # prologue (constant-bank fetch, SM dispatch) while our tail finishes.
