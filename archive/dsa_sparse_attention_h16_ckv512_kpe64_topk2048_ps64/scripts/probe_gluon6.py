"""Sixth probe: DotOperandLayout k_width=None over BlockedLayout."""

import modal

app = modal.App("gluon-probe6")
...
def probe():
    ...
    # Now bf16 (more realistic)
    print("\n=== bf16 dot_fma k_width=None ===")
    @gluon.jit
    ...
    try:
        matmul_bf16[(1,)](a, b, c, M, N, K, num_warps=8)
        c_ref = (a.float() @ b.float()).to(torch.bfloat16)
        print(f"bf16 match: {torch.allclose(c, c_ref, atol=1e-1, rtol=1e-1)}")
